...
- Detecting scoops vs. errors among rare facts (see [§7.2.2](#722-rarity-weighting-tf-idf-style)).
...
Takes all facts from all *k* articles and clusters them by cosine similarity. Implementation: agglomerative clustering with cosine distance and threshold τ (default 0.15 → similarity > 0.85). Returns a `FactUniverse` object holding:
...
- `centrality(article_embeddings)` — center embeddings, return normalized $-\|e_i - \mu\|$.
...
Default values for: similarity threshold τ, weighting mode (`consensus` vs `rarity`), final weights (α, β, γ, δ), embedding model name, LLM model name, cache directory.
...
This section contains all formulas in the order the pipeline computes them, with explanations for each.
...
| **τ** | cosine similarity threshold for fact deduplication (default 0.85) |
| **‖·‖** | Euclidean (L2) norm |
| **⟨·,·⟩** | dot product |
...
#### 7.1.2 Fact clustering (agglomerative, single-link)

Two facts belong to the same cluster if:

$$
\text{sim}(f_a, f_b) > \tau
$$

Each cluster collapses to one canonical fact. The fact universe **F** is the set of canonical facts.

> **Why:** Different articles describe the same fact with different words ("300 protesters gathered" vs "hundreds rallied"). Clustering merges these so we count the underlying fact once. Single-link agglomerative clustering chains paraphrases together, which is what we want — A ≈ B and B ≈ C should put A, B, C in one cluster even if A and C have lower direct similarity. The threshold τ is the main knob: too low merges distinct facts, too high over-counts the same fact as multiple.
...
$$
w_j^{\text{cons}} = \frac{\sum_{i=1}^{k} C_{ij}}{k}
$$
...
#### 7.2.2 Rarity weighting (TF-IDF style — rewards scoops)

$$
w_j^{\text{rare}} = \log\!\left(\frac{k}{\sum_{i=1}^{k} C_{ij}}\right)
$$

> **Why:** Inverse-document-frequency style. A fact in only 1 article gets the highest weight (log k); a fact in all k articles gets weight 0. This rewards articles with **unique, hard-to-find information**. **Caveat:** rarity weighting is risky because rarity ≠ correctness — a unique fact could be a scoop, a minor detail, or an error. Use this mode only when you have additional signals (source quality, attribution, semantic relatedness to the core story) to validate rare facts.
...
$$
\tilde{\text{dens}}_i = \frac{U_i}{E_i + L_i}
$$
...
For each component **x ∈ {c̃, c̃ov, d̃ens, ẽnt}**, normalize across the *k* articles:

$$
x_i = \frac{\tilde{x}_i - \min_j \tilde{x}_j}{\max_j \tilde{x}_j - \min_j \tilde{x}_j + \varepsilon}
$$

where ε is a small constant (e.g. 10⁻⁹) to avoid division by zero when all articles are tied.

After this step: $c_i, \text{cov}_i, \text{dens}_i, \text{ent}_i \in [0, 1]$.

> **Why:** The four components live on different scales. Centrality is a negative distance (could be −0.3 to −1.2); coverage is a fraction in [0, 1]; density is also in [0, 1] but rarely uses the full range. Min-max normalization rescales each to [0, 1] across the k articles so the weighted sum is meaningful. The ε guard prevents NaN when all articles tie on a component (e.g. all identical). An alternative would be z-score normalization, but min-max is preferable here because it bounds the output for the final weighted sum.
...
- **Threshold τ.** Default 0.85, but **tune by inspection** on a small dev corpus before committing. Print clusters and eyeball.
...
**Identity 1 — Identical articles.** If all articles are identical, all centralities should be 1.0 (they're all at the centroid), all coverages should be 1.0 (they all cover everything), the ranking is degenerate. Min-max normalization will return NaNs without the ε guard.
...
**Identity 3 — Reduction to original idea.** If you set α = 1 and β = γ = δ = 0, the ranking reduces to the original "most centered article wins" idea.

**Identity 4 — Density bound.** $U_i \leq E_i + L_i$ always holds, so raw density is already in [0, 1] before normalization.
