...
- `news_ranker/decompose.py` — provider-agnostic `decompose()` flow with injected client, Mistral default model name, parse/validation retry, and disk cache.
- `news_ranker/mistral.py` — provider-specific Mistral adapter implementing the injected decomposition client boundary.
...
- `news_ranker/config.py` — config knobs relevant here: embedding/LLM model names, prompt/schema versions, cache dir, similarity/linkage settings; LLM default uses Mistral model naming but remains metadata.
...
- `tests/test_decompose.py` — fake-client decomposition success, Mistral default-model forwarding, input validation, retry behavior, cache hits, cache key versioning, no network/provider calls.
- `tests/test_mistral.py` — fake Mistral SDK tests for adapter messages, API-key resolution, response text extraction, decompose integration, and cache hit behavior.
...
- `tests/test_health.py` — public import surface includes generic decomposition exports but not result/evaluate helpers or `MistralDecompositionClient`.
...
- `docs/plans/mistral-llm-provider.md` — plan that added Mistral dependency, adapter, model-default alignment, README example, and submodule-only import tests.
- `docs/context/mistral-llm-provider.md` — companion context for Mistral-specific SDK adapter behavior.
...
5. Decomposition cache is optional. When `cache_dir` is set, cache path is `cache_dir/decompositions/<sha256>.json`; hash includes normalized raw article payload, model, prompt version, and schema version. Cache hit bypasses client, validates cached JSON, and still applies runtime article ID from raw input. Default-model cache keys now include `"mistral-small-latest"`.
...
- Provider dependencies are injected. Mistral SDK integration lives in `news_ranker.mistral`; tests fake that boundary and must not call hosted providers, download models, or instantiate real `SentenceTransformer`.
...
- Cache invalidation depends on raw article payload plus model/prompt/schema version strings; prompt/schema constants live outside `RankerConfig` defaults. `RankerConfig.llm_model_name` matches Mistral naming but is not used by `decompose()` automatically.
- Public package exports currently include generic decomposition entries. Result/evaluation dataclasses/helpers and `MistralDecompositionClient` remain submodule-only.
...
Plan session should focus on whether to further verify the Mistral SDK boundary or wire config metadata into decomposition while preserving current strict fixture schema and injected-dependency tests.
